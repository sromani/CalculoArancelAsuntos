  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {eliminar ? (
        ...
        >
          <div
            ...
            className="max-h-[85dvh] w-full max-w-md overflow-y-auto overflow-x-hidden rounded-xl border border-[rgba(0,166,81,0.22)] bg-white p-4 shadow-xl sm:max-h-[90dvh] sm:p-5"
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => e.stopPropagation()}
          >
            <h2 id="eliminar-titulo" className="text-lg font-semibold text-[var(--verde-titulo)]">
              {eliminar.paso === 1 ? "Confirmar eliminacion" : "Ultima confirmacion"}
            </h2>
            {eliminar.paso === 1 ? (
              <p className="mt-3 text-sm text-[var(--gris-texto)]/90">
                ¿Eliminar <span className="font-semibold text-[var(--verde-titulo)]">{eliminar.etiqueta}</span>?
              </p>
            ) : (
              <p className="mt-3 text-sm text-[var(--gris-texto)]/90">
                Segunda confirmación: se eliminará permanentemente{" "}
                <span className="font-semibold text-[var(--verde-titulo)]">{eliminar.etiqueta}</span>.
              </p>
            )}
            {eliminar.paso === 1 && cargandoPendientes ? (
              <p className="mt-2 text-sm text-[var(--gris-texto)]/90">Comprobando asuntos en trámite…</p>
            ) : null}
            {eliminar.paso === 1 &&
            !cargandoPendientes &&
            pendientesMiembro &&
      ...
      {editSocio ? (
        ...
        >
          <div
            ...
            onKeyDown={(e) => e.stopPropagation()}
          >
            <h2 className="text-lg font-semibold text-[var(--verde-titulo)]">Editar socio</h2>
            <p className="mt-1 text-sm text-[var(--gris-texto)]">Rol: Socio (fijo).</p>
            <form className="mt-4 space-y-3" onSubmit={(e) => void guardarEditSocio(e)}>
              <div>
                <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]">Nombre *</label>
                <input
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]">
                  Profesión (opcional)
                </label>
                <input
                  className={inputClass}
              ...
              <div className="flex flex-wrap justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={guardEditSocio}
                  className="rounded-md bg-[var(--verde-titulo)] px-3 py-1.5 text-xs font-medium text-white hover:bg-[var(--verde-oscuro)] disabled:opacity-60"
                >
      ...
      {editProf ? (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            className="max-h-[85dvh] w-full max-w-lg overflow-y-auto overflow-x-hidden rounded-xl border border-[rgba(0,166,81,0.22)] bg-white p-4 shadow-xl sm:max-h-[90dvh] sm:p-5"
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => e.stopPropagation()}
          >
            <h2 className="text-lg font-semibold text-[var(--verde-titulo)]">Editar miembro del equipo</h2>
            ...
            <form className="mt-4 space-y-3" onSubmit={(e) => void guardarEditProf(e)}>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]">Rol *</label>
                <select
                  className={inputClass}
                  value={editProfRol}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]">
                  ...
                  {!puestoRequiereFuncionEnEstudio(editProfRol) ? (
                    <span className="font-normal text-[var(--gris-texto)]"> (opcional en dirección)</span>
                  ) : null}
                </label>
                <input
                  className={inputClass}
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:gap-8">
        <div className="space-y-4 rounded-xl border border-[rgba(0,166,81,0.28)] bg-white p-4 shadow-sm shadow-[rgba(0,166,81,0.06)] sm:p-5">
          <div>
            <h2 className="text-lg font-semibold text-[var(--verde-titulo)]">Alta de Socio</h2>
            <p className="mt-1 text-sm text-[var(--gris-texto)]">
          ...
          <form className="space-y-3" onSubmit={(e) => void onSubmitSocio(e)}>
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-soc-nombre">
                Nombre *
              </label>
              <input
                id="alta-soc-nombre"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-soc-prof">
                Profesión (opcional)
              </label>
              <input
                id="alta-soc-prof"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-soc-func">
                Función en el estudio (opcional)
              </label>
              <input
                id="alta-soc-func"
            ...
            <button
              type="submit"
              disabled={guardSoc}
              className="rounded-lg bg-[var(--verde-titulo)] px-4 py-2 text-sm font-medium text-white hover:bg-[var(--verde-oscuro)] disabled:opacity-60"
            >
        ...
        <div className="space-y-4 rounded-xl border border-[rgba(0,166,81,0.28)] bg-white p-4 shadow-sm shadow-[rgba(0,166,81,0.06)] sm:p-5">
          <div>
            <h2 className="text-lg font-semibold text-[var(--verde-titulo)]">Alta de Equipo</h2>
            <p className="mt-1 text-sm text-[var(--gris-texto)]">
          ...
          <form className="space-y-3" onSubmit={(e) => void onSubmitEquipo(e)}>
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-eq-nombre">
                Nombre *
              </label>
              <input
                id="alta-eq-nombre"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-eq-rol">
                Rol *
              </label>
              <select
                id="alta-eq-rol"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-eq-prof">
                Profesión (opcional)
              </label>
              <input
                id="alta-eq-prof"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--verde-titulo)]" htmlFor="alta-eq-func">
                Función en el estudio
                {!puestoRequiereFuncionEnEstudio(eqRol) ? (
                  <span className="font-normal text-[var(--gris-texto)]"> (opcional)</span>
                ) : (
      ...
      <div className="rounded-xl border border-[rgba(0,166,81,0.28)] bg-white p-4 shadow-sm shadow-[rgba(0,166,81,0.06)] sm:p-5">
        <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
          ...
          <div className="w-full sm:w-auto sm:min-w-[220px] sm:max-w-sm">
            <label
              className="mb-1 block text-xs font-medium text-[var(--gris-texto)]/90"
        ...
        {cargando ? (
          ...
          <p className="text-sm text-[var(--gris-texto)]">No hay registros.</p>
              ...
              <tbody>
                {filasListaFiltradas.map((row) => (
                  <tr key={row.key} className="border-b border-[rgba(0,166,81,0.14)]">
                      ...
                      <span
                        className={
                          row.tipo === "socio"
                            ...
                            : "rounded bg-[rgba(0,166,81,0.12)] px-2 py-0.5 text-xs font-medium text-[var(--gris-texto)]"
                        }
                      >
                    ...
                    <td className="py-2.5 pr-3 text-[var(--gris-texto)]/90">{row.rolEtiqueta}</td>
                    <td className="py-2.5 pr-3 text-[var(--gris-texto)]/90">{row.areaEtiqueta}</td>
                    <td className="max-w-[140px] truncate py-2.5 pr-3 text-[var(--gris-texto)]/90" title={row.profesion}>
                      {row.profesion || "—"}
                    </td>
                    <td className="max-w-[160px] truncate py-2.5 pr-3 text-[var(--gris-texto)]/90" title={row.funcion}>
                      {row.funcion || "—"}
                    </td>
