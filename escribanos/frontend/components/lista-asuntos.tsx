...
const filtroBox =
  "rounded-xl border border-[rgba(0,166,81,0.22)] bg-gradient-to-br from-white to-[var(--fondo-verde-claro)]/50 p-3 shadow-sm shadow-[rgba(0,166,81,0.06)] sm:p-4";

const labelFiltro =
  "mb-1.5 block text-xs font-semibold uppercase tracking-wide text-[var(--verde-subtitulo)]";
    ...
    <div className="card-app min-w-0 space-y-6 rounded-2xl p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <h2 className="text-lg font-semibold text-[var(--verde-titulo)] sm:text-xl">Listado</h2>
          <p className="mt-1 text-base leading-relaxed text-[var(--gris-texto)]">
            Filtrá por estado, tipo, equipo o fechas. Tu rol:{" "}
            <span className="font-semibold text-[var(--verde-titulo)]">{rol ?? "…"}</span>
          </p>
        </div>
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          {!cargando ? (
            <span className="inline-flex items-center rounded-full bg-[rgba(0,166,81,0.12)] px-3 py-1.5 text-sm font-medium text-[var(--verde-titulo)]">
          ...
          {hayFiltrosActivos ? (
            <button
              type="button"
              className="inline-flex min-h-[2.75rem] items-center justify-center rounded-[10px] border-2 border-[rgba(0,166,81,0.35)] bg-white px-4 py-2 text-sm font-semibold text-[var(--gris-texto)] shadow-sm transition-colors hover:bg-[var(--fondo-verde-muy-claro)]"
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className={filtroBox}>
          <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-[var(--verde-titulo)]">
            Búsqueda y tipo
          </p>
        ...
        <div className={filtroBox}>
          <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-[var(--verde-titulo)]">Equipo</p>
          <div className="grid gap-3 sm:grid-cols-2">
            <label>
        ...
        <div className={`${filtroBox} lg:col-span-2`}>
          <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-[var(--verde-titulo)]">Fechas</p>
      ...
      {cargando ? (
        <div className="flex items-center gap-3 rounded-xl border border-[rgba(0,166,81,0.12)] bg-white/80 px-4 py-6 text-sm text-[var(--gris-texto)]/90">
          <span
            className="inline-block size-5 animate-spin rounded-full border-2 border-[rgba(0,166,81,0.2)] border-t-[var(--verde-principal)]"
            aria-hidden
          />
          Cargando asuntos…
        </div>
      ) : lista.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[rgba(0,166,81,0.2)] bg-[var(--fondo-verde-claro)]/40 px-6 py-10 text-center">
          <p className="text-base font-medium text-[var(--verde-titulo)]">No hay asuntos con estos filtros</p>
          <p className="mt-2 text-sm text-[var(--gris-texto)]">
            {hayFiltrosActivos
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            {hayFiltrosActivos ? (
              <button
                type="button"
                className="inline-flex min-h-[2.75rem] items-center justify-center rounded-[10px] border-2 border-[rgba(0,166,81,0.35)] bg-white px-5 py-2 text-sm font-semibold text-[var(--gris-texto)] hover:bg-[var(--fondo-verde-muy-claro)]"
                onClick={limpiarFiltros}
              >
                Limpiar filtros
              </button>
        ...
        <>
          <div className="hidden min-w-0 rounded-xl border border-[rgba(0,166,81,0.28)] bg-white shadow-sm shadow-[rgba(0,166,81,0.06)] md:block">
            <div className="overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch]">
              <table className="w-full min-w-[820px] text-left text-sm text-[var(--color-neutral-900)]">
                <thead>
                ...
                <tbody>
                  {lista.map((a) => (
                    ...
                    >
                      <td className="px-4 py-3 font-mono text-sm text-[var(--verde-titulo)]">{a.ordinal}</td>
                      ...
                      <td className="px-4 py-3 text-[var(--verde-titulo)]">{etiquetaTipo(a.tipo)}</td>
                      <td className="max-w-[200px] px-4 py-3">
                        <span className="font-medium text-[var(--verde-titulo)]">{a.cliente.nombre}</span>
                        <span className="mt-0.5 block truncate text-xs text-[var(--verde-subtitulo)]/80">{a.cliente.documento}</span>
                      </td>
                      <td className="max-w-[220px] truncate px-4 py-3 text-[var(--verde-titulo)]" title={a.catalogo.nombre}>
                        {a.catalogo.nombre}
                      </td>
                      <td className="max-w-[160px] truncate px-4 py-3 text-[var(--gris-texto)]" title={a.profesionalACargo.nombre}>
                        {a.profesionalACargo.nombre}
                      </td>
                      <td className="whitespace-nowrap px-4 py-3 text-[var(--gris-texto)]">{fmtFechaCorta(a.fechaInicio)}</td>
                      <td className="px-4 py-3 text-right">
                        <Link
                          className="inline-flex min-h-[2.5rem] min-w-[6.5rem] items-center justify-center rounded-[10px] bg-[var(--verde-principal)] px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[var(--verde-oscuro)]"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="font-mono text-sm text-[var(--verde-principal)]">#{a.ordinal}</p>
                    <p className="mt-1 text-base font-semibold leading-snug text-[var(--verde-titulo)]">{a.catalogo.nombre}</p>
                    <p className="mt-2 text-sm leading-relaxed text-[var(--gris-texto)]">{a.cliente.nombre}</p>
                    <p className="mt-0.5 text-sm text-[var(--gris-texto)]">{a.cliente.documento}</p>
                  </div>
                  <span
                    className={
                      a.estado === "FINALIZADO"
                ...
                <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-t border-[rgba(0,166,81,0.12)] pt-4 text-sm leading-relaxed text-[var(--gris-texto)]">
                  <span>
                    <span className="text-[var(--verde-subtitulo)]/80">Tipo:</span> {etiquetaTipo(a.tipo)}
                  </span>
                  <span>
                    <span className="text-[var(--verde-subtitulo)]/80">Equipo:</span> {a.profesionalACargo.nombre}
                  </span>
                  <span>
                    <span className="text-[var(--verde-subtitulo)]/80">Inicio:</span> {fmtFechaCorta(a.fechaInicio)}
                  </span>
                </div>
                <Link
                  className="btn-primary mt-4 inline-flex min-h-[3rem] w-full items-center justify-center rounded-[10px] border-2 border-transparent px-6 py-3 text-center text-base font-semibold shadow-md shadow-[rgba(0,166,81,0.15)]"
                  href={`/estudio/asuntos/${a.id}`}
                >
                  Ver ficha
                </Link>
