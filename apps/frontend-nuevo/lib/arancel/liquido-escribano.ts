...
/**
 * Desglose para presupuesto con honorario manual.
 * Si honorario ≥ arancel: recalcula montepío y demás aportes sobre honorario a cobrar.
 * Si honorario < arancel: montepío/fondo gremial no bajan del cálculo por arancel (piso).
 */
export function calcularDesglosePresupuestoHonario(
  ...
  fonasaPct: number,
  irpfPct: number
): { lineasArancel: LineasDesgloseLiquido; lineasPresupuesto: LineasDesgloseLiquido } | null {
  const lineasArancel = calcularDesgloseLiquido(
    honorarioArancel,
    moneda,
    tasas,
    fonasaPct,
  ...
  if (!lineasArancel) return null;

  const hArancel = lineasArancel.honorario;
  const hCobrar = honorarioPrincipalHaciaArriba(honorarioACobrar);

  let lineasPresupuesto: LineasDesgloseLiquido | null;

  if (hCobrar >= hArancel) {
    lineasPresupuesto = calcularDesgloseLiquido(hCobrar, moneda, tasas, fonasaPct, irpfPct);
  } else {
    lineasPresupuesto = calcularDesgloseLiquidoConAportesArancel(
      hCobrar,
      {
        montepio: lineasArancel.montepio,
        fondoGremial: lineasArancel.fondoGremial,
        fondoReconversionLaboral: lineasArancel.fondoReconversionLaboral,
        totalAportes: lineasArancel.totalAportes,
      },
      moneda,
      tasas,
      fonasaPct,
  ...
  if (!lineasPresupuesto) return null;
  return { lineasArancel, lineasPresupuesto };
}
