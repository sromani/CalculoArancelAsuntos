...
export type SimuladorResultado = {
  acto: { key: string; nombre: string; articulo: string };
  resultado: {
...
export type PresupuestoRow = {
  ...
  lineasGastos: {
    ...
    moneda: MonedaGasto;
    incluido: boolean;
    gasto: { id: string; nombre: string; categoria: CategoriaGasto; estado: EstadoGasto };
  }[];
};

export const ETIQUETA_ESTADO_PRESUPUESTO: Record<import("./enums").EstadoPresupuesto, string> = {
  BORRADOR: "Borrador",
  EMITIDO: "Emitido",
  ACEPTADO: "Aceptado",
  RECHAZADO: "Rechazado",
